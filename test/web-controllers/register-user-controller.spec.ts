...
describe('Register user web controller', () => {
  ...
  const sendEmailUseCase: SendEmail = new SendEmail(
    mailOptions,
    mailServiceStub,
  );
  ...
  const controller: RegisterAndSendEmailController =
    new RegisterAndSendEmailController(registerAndSendEmailUseCase);
  ...
  test('should return status code 500 when server raises', async () => {
    ...
    const controller: RegisterAndSendEmailController =
      new RegisterAndSendEmailController(errorThrowingUseCaseStub);
    const response: HttpResponse = await controller.handle(request);
